  ...
  return (
    <section className="py-16 md:py-20 bg-transparent relative border-t border-border/50">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Blog Grid */}
        <StaggerContainer staggerChildren={0.2} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {posts.map((post, index) => (
            <StaggerItem key={index} direction="up" className="h-full">
              <article 
                className={`group flex flex-col p-6 rounded-2xl bg-card border border-border/50 hover:border-primary/50 transition-all duration-300 h-full`}
              >
